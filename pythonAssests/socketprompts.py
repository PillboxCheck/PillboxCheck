...
HALLUCINATION_GRADER_PROMPT = PromptTemplate(
    template="""Role: Hallucination Grader. You are a grader evaluating whether an answer is grounded in the provided facts.
...
5. ** Important Rule:** If the User is not involved in the question ignore between "[USERINFORMATION]" and "[END USERINFORMATION]"
[END GUIDELINES]
...
INTERNAL_MEDSCAN_PROMPT = PromptTemplate(
    template="""
    [GUIDELINES]
...
INTERNAL_EVENTSCAN_PROMPT = PromptTemplate(
    template="""
    [GUIDELINES]
    You are given a [DOCUMENT] and must extract all Events or Appointments it contains.

    Return your answer **only** as a JSON object in the following format:
    {{
      "events": [
        {{
          "Name": "Event name as a string",
          "ScheduledTimeSTR": "Event date and time in 'yyyy-MM-dd HH:mm:ss' format",
          "Location": "Event location as a string"
        }},
        ...
      ]
    }}

    - **Name**: The title or description of the event.
    - **ScheduledTimeSTR**: The event's scheduled date and time (formatted as "yyyy-MM-dd HH:mm:ss").
    - **Location**: Where the event will occur.

    **Rules**:
    - If any field information is missing, set its value to `null`.
    - Output **only** the JSON object — no explanations, summaries, or extra text.
