...
int main(int argc, char** argv) {
    ...
    //     )";
    const char* system_message = R"(You are a helpful assistant. Please provide a response to the user's query.
        You are an AI assistant with expertise in multiple roles. Your responses must strictly follow the specific **guidelines** provided for each task.
        Do not give any explanations or opinions. Only respond to the User section of the prompt.)";
    while (true) {
        ...
        if (user_input == "exit") {
         ...
         // Create the sampler chain once (it is independent of the model/context)
        ...
        // For each message, load and initialize the model and its context.
        ...
        // Free the dynamically allocated user message.
        for (size_t i = 1; i < current_messages.size(); i++) {
            free((void*)current_messages[i].content);
        }
